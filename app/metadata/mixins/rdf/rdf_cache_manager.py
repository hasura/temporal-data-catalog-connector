...
class RDFCacheManager:
    """
    Manages caching for RDF metadata graphs, supporting both model-level and instance-level caching.
    """

    def __init__(
            self,
            cache_dir: str = './.rdf_cache',
            model_maxsize: int = 128,  # MB for model metadata
            instance_maxsize: int = 32,  # MB for instance metadata
        ...
        Args:
            ...
            model_maxsize: Maximum size in MB for model metadata cache
            instance_maxsize: Maximum size in MB for instance metadata cache
    ...
    def get_model_metadata(
        ...
        """
        Get model metadata from cache or generate if not present.

        Args:
            cls: The model class
            generator_func: Function to generate model metadata if not cached
        """
    ...
    def get_instance_metadata(
        ...
        """
        Get instance metadata from cache or generate if not present.

        Args:
            instance: Model instance
            session: Session
            generator_func: Function to generate instance metadata if not cached
        """
    ...
    def get_bulk_instance_metadata(
        ...
        """
        Get bulk instance metadata from cache or generate if not present.

        Args:
            cls: The model class
            session: SQLAlchemy session
            generator_func: Function to generate bulk instance metadata if not cached
        """
    ...
    def _generate_model_key(cls: type) -> str:
        """Generate a cache key for model-level metadata"""
        ...
        logger.debug(f"Generated model metadata cache key: {key}")
        return key
    ...
    def _generate_bulk_instance_key(cls: type, session: Session) -> str:
        """Generate a cache key for bulk instance metadata"""
        ...
        # logger.debug(f"Generated bulk instance metadata cache key: {key}")
        return key
