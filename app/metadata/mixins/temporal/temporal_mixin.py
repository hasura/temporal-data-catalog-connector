...
def handle_temporal_tracking(session: Session, context, instances) -> None:
    ...
    for obj in session.new | session.dirty:
        if isinstance(obj, TemporalMixin):
            ...
            # Build identifier string from business keys for logging
            try:
                identifier = ', '.join(f"{k}={getattr(obj, k)}" for k in obj.get_business_keys())
                logger.debug(f"Processing {obj.__class__.__name__}({identifier})")
            except Exception:
                identifier = "unknown"

            # For new objects, check for existing records using business keys
            if obj in session.new:
                try:
                    # Build business key filter
                    business_key_filter = obj.build_business_key_filter()

                    # Check session first using business keys
                    existing = None
                    for session_obj in session:
                        if (isinstance(session_obj, obj.__class__) and
                                session_obj not in session.new and
                                all(getattr(session_obj, key) == value
                                    for key, value in business_key_filter.items())):
                            existing = session_obj
                            break

                    # If not in session, query DB using business keys
                    if not existing:
                        existing = session.query(obj.__class__).filter_by(
                            **business_key_filter,
                            t_is_current=True
                        ).first()
                    ...
                    logger.warning(
                        f"Unable to check business keys for {obj.__class__.__name__}({identifier}): "
